...
import { clearData, setNotEmpty, show } from "../../redux/actions";
import {
  fetchData,
  selectSelector,
  showBillSelector,
...
const DrinksList = () => {
  ...
  const list = useSelector(showListSelector);
  const [nc, setNC] = useState(true);
  const dispatch = useDispatch();
  const handlePrintBill = () => {
    dispatch(show({ list: false, bill: false }));
    dispatch(setNotEmpty(currentTable));
  };
  const handleSetEmpty = () => {
    dispatch(show({ list: false, bill: false }));
    dispatch(clearData(currentTable));
  };
  const handleClose = () => {
    dispatch(clearData(currentTable));
    dispatch(show({ list: false, bill: false }));
              ...
              <div key={item.id} className=" w-full">
                ...
                <div className="grid grid-cols-3 gap-2 pt-2 w-full">
                  {filterCategories(item.title) &&
                    filterCategories(item.title).map((drink) => (
                      <Drink
                        key={drink.id}
                        imgUrl={drink.imgUrl}
                        name={drink.name}
